...
wb=xw.Book()
sht=wb.sheets[0]
sht.name="Analysis of brands store-wise"
...
def avg_fullness(date,sheet_name,brand):
...
def critical_pts(sheet_name,brand):
    df=file_extract(sheet_name)
    ...
    fullness_list=[]
    for dates in dt_lss:
        fullness_list.append(avg_fullness(dates,sheet_name,brand))
    ...
    while i<(len(cust_full)-1):
        if cust_full[i+1]-cust_full[i]>5:
            if cust_full[i+1] in critical_ls and abs(cust_full[i+1]-cust_full[i]) >5 :
                ...
                # date_rp
                full_dev.append(str(diff_round))  
    ...
    plt.title(f"Analysis of Vol. Rates of {brands} in {sheet_name}")
    plt.legend()
    ...
    sht.pictures.add(
        fig,name=f"bar_representation_{sheet_name}_{ref}",
        update=False,
        left=sht.range(ref+str(k)).left,
        top=sht.range(ref+str(k)).top,
        height=200,
        width=250,
    )
    ...
    plt.plot(m_cust_dates,cust_full,label="Koram Mall",color="y")
    ...
    plt.title(f"Analysis of fullness of {brands}")
    ...
    sht.pictures.add(
        fig,name=f"Graphical Rep_{sheet_name}_{ref}",
        update=False,
        left=sht.range(ref1+str(k)).left,
        top=sht.range(ref1+str(k)).top,
        height=200,
        width=250,
    )
...
# df_new.loc[0]=["Trends-Bandra-Mumbai",1.4,1.43,11.12,15.36,"59.15%"]
# df_new.loc[1]=["Trends-Koram Mall-Mumbai",1.22,1.22,8.81,8.7,"44.35%"]
# df_new.loc[2]=["Trends-Imperial Mall,PCMC-Pune",1.5,1.0,15.93,6.29,"67.27%"]
# df_new.loc[3]=["Trends-Amanora-Pune","No replenishment",2.0,"No replenishment",7.62,"69.49%"]
# df_new.loc[4]=["Trends-Market City-Pune","No replenishment",4.0,"No replenishment",10.86,"38.9%"]
# df_new.loc[5]=["Trends-RIL Mall,Elpro-Pune",2.75,1.0,11.61,11.23,"42.39%"]
# df_new.loc[6]=["Trends-Seawoods-NaviMumbai",8.0,4.0,10.14,10.09,"77.78%"]
# df_new.loc[7]=["Trends-DVictoria-Mumbai",2.6,2.17,18.42,12.47,"34.04%"]
# # print(df_new)
...
def remove_files():
    removing_files = glob.glob('C:\\Users\\Arpit Mohanty\\analytics/*.png')
...
for i in range(len(sheets)):
    counter=0
    a=file_extract(sheets[i])
    ref="A"
    ref1="N"
    df_new=pd.DataFrame(columns=["Trends Beauty Stores","Brand","MTTR(Days)","MTTC(Days)","Replenishment Rate(Vol)","Consumption Rate(Vol)","Avg.Shelf-Fullness"])

    brands="Lakme"
    # unique_br=[]
    
    try:
        a,b,c,d,e=critical_pts(sheets[i],"Lakme")
        
        df_new.loc[counter]=[sheets[i],brands,a,b,c,d,str(e)+"%"]
        ref="H"
        ref1="T"
        brands="Flormar"
        counter=1
        a,b,c,d,e=critical_pts(sheets[i],"Flormar")
        df_new.loc[counter]=[sheets[i],brands,a,b,c,d,str(e)+"%"]
        k+=20

        sht["J"+str(k)].options(pd.DataFrame, header=1, index=True, expand='table').value = df_new
        sht["J"+str(k)].expand("right").api.Font.Bold = True
        sht["J"+str(k)].expand("down").api.Font.Bold = True
        sht["J"+str(k)].expand("right").api.Borders.Weight = 2
        sht["J"+str(k)].expand("down").api.Borders.Weight = 2
